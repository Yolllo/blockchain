...
func (scf *systemSCFactory) createCustomStakingContract() (vm.SystemSmartContract, error) {
	argsCustomStaking := systemSmartContracts.ArgsNewCustomStaking{
		Eei:     scf.systemEI,
		GasCost: scf.gasCost,
	}
	customStaking, err := systemSmartContracts.NewCustomStakingSystemSC(argsCustomStaking)
	return customStaking, err
}
...
func (scf *systemSCFactory) CreateForGenesis() (vm.SystemSCContainer, error) {
	...
	customStakingSC, err := scf.createCustomStakingContract()
	if err != nil {
		return nil, err
	}

	err = scf.systemSCsContainer.Add(vm.CustomStakingSCAddress, customStakingSC)
	if err != nil {
		return nil, err
	}
